...
public class CalendarAction extends ActionSupport {
	...
	private List<windowVO> winlist = new ArrayList<windowVO>();

	private boolean finish = false;

	public CalendarAction() throws IOException {
	...
	public String execute() throws SQLException, ParseException {
		...
		calendHtml.append("<td  align = 'center' bgcolor='#013e97' style='background:url(./image/bgcolor.png)' ><a style='color:black' href='/KHI/CalendarAction.action?year=" + year + 
				"&month=" + (month - 1) + "&date=" + date +"'> <img src='./image/left.png'width='15' height='15'> </a></td>");
		
		calendHtml.append("<th bgcolor='#013e97' height='40'  colspan=5 style='background:url(./image/bgcolor.png)' ><font color='#ffffff' size='4'>" + year + "년" + (month + 1) + "월"+ "</font></th>");
		
		calendHtml.append("<td style='background:url(./image/bgcolor.png)' bgcolor='#013e97' align = 'center'><a style='color:black'  href='/KHI/CalendarAction.action?year=" + year + "&month=" + (month + 1) + "&date=" + date + "'><img src='./image/right.png'width='15' height='15'> </a></td>");
		...
		for (double i = 1; i <= c; i++) {
			...
			for (int j = 1; j <= 7; j++) {
				...
				try {
					day2 = dateFormat.parse(year + "-" + (month + 1) + "-" + (dayNum));
					day3 = dateFormat.parse(year + "-" + (month + 1) + "-" + (dayNum + 1));

					day.setFirstday(new Date(day2.getTime()));
					day.setEndday(new Date(day3.getTime()));
					
					if(D_day.getTime() >= day2.getTime() ) {
						tempCount = (Integer) sqlMapper.queryForObject("ReservDay_count", day);
					}else
					{
						tempCount = (Integer) sqlMapper.queryForObject("ReservDayOver_count", day);
					}
		...
		winlist = sqlMapper.queryForList("windowALL");
	...
	public static void setSqlMapper(SqlMapClient sqlMapper) {
		CalendarAction.sqlMapper = sqlMapper;
	}
	...
	public List<windowVO> getWinlist() {
		return winlist;
	}

	public void setWinlist(List<windowVO> winlist) {
